...
# pipeline.transformer.model.transformer_high.blocks = torch.nn.ModuleList([original_blocks[i] for i in range(0,1)]) # 2 layers
# pipeline.transformer.model.transformer_low.blocks = torch.nn.ModuleList([org_blocks[i] for i in range(0,1)]) # 2 layers
...
export_to_video(frames, "output_t2v_480p.mp4", fps=16)
print(output)
