...
# Replicate the conditional `emb` calculation from the original module's forward.
        ...
        if self.emb_module is not None:
            ...
            if emb is None:
                raise ValueError("External 'emb' tensor must be provided when original module has no CombinedTimestepLabelEmbeddings.")
            emb_for_linear = emb
...
class GELUFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        if projected_states.device.type == "mps" and projected_states.dtype == torch.float16:
            ...
            output = F.gelu(projected_states.to(dtype=torch.float32), approximate=approximate_type).to(dtype=projected_states.dtype)
        else:
...
class GELUAIC(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if proj_bias_to_pass is None:
            ...
            proj_bias_to_pass = torch.zeros(self.proj_weight.shape[0], device=hidden_states.device, dtype=hidden_states.dtype)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if act_fn_proj_bias_to_pass is None:
            act_fn_proj_bias_to_pass = torch.zeros(self.act_fn_proj_weight.shape[0], device=device, dtype=dtype) # Use proper shape
        
        project_out_bias_to_pass = self.project_out_bias
        if project_out_bias_to_pass is None:
            project_out_bias_to_pass = torch.zeros(self.project_out_weight.shape[0], device=device, dtype=dtype) # Use proper shape
...
# Apply RMSNorm if enabled (norm_q, norm_k)
        ...
        if actual_encoder_hidden_states is not None:
            encoder_hidden_states_query_proj = F.linear(actual_encoder_hidden_states, add_q_proj_weight, add_q_proj_bias)
            encoder_hidden_states_key_proj = F.linear(actual_encoder_hidden_states, add_k_proj_weight, add_k_proj_bias)
            encoder_hidden_states_value_proj = F.linear(actual_encoder_hidden_states, add_v_proj_weight, add_v_proj_bias)
            ...
            encoder_hidden_states_query_proj = RMSNormFunc.apply(encoder_hidden_states_query_proj, norm_added_q_weight, norm_added_q_eps, norm_added_q_elementwise_affine)
            encoder_hidden_states_key_proj = RMSNormFunc.apply(encoder_hidden_states_key_proj, norm_added_k_weight, norm_added_k_eps, norm_added_k_elementwise_affine)
...
class JointAttnProcessor2_0AIC(nn.Module): 
        ...
        def get_param_or_dummy_zero(param, default_shape_dim=None):
            ...
            if param is None: # It's None, create a dummy zero tensor
                if default_shape_dim is not None:
                    ...
                    if isinstance(default_shape_dim, int): return torch.zeros(default_shape_dim)
                    # For weights of RMSNorm, shape is (dim,)
                    if isinstance(default_shape_dim, tuple): return torch.zeros(default_shape_dim)
 ...
 # RMSNorm parameters (for norm_q, norm_k, norm_added_q, norm_added_k)
        ...
        self.add_q_proj_weight = get_param_or_dummy_zero(attn_module.add_q_proj.weight, attn_module.add_q_proj.out_features)
        self.add_q_proj_bias = get_param_or_dummy_zero(attn_module.add_q_proj.bias, attn_module.add_q_proj.out_features)
        self.add_k_proj_weight = get_param_or_dummy_zero(attn_module.add_k_proj.weight, attn_module.add_k_proj.out_features)
        self.add_k_proj_bias = get_param_or_dummy_zero(attn_module.add_k_proj.bias, attn_module.add_k_proj.out_features)
        self.add_v_proj_weight = get_param_or_dummy_zero(attn_module.add_v_proj.weight, attn_module.add_v_proj.out_features)
...
# Other flags from Attention
        ...
        self.to_add_out_weight = get_param_or_dummy_zero(attn_module.to_add_out.weight, attn_module.to_add_out.out_features)
    ...
    # Determine if encoder_hidden_states and attention_mask were originally None
        ...
        _original_encoder_hidden_states_was_none = (encoder_hidden_states is None) or (encoder_hidden_states.numel() == 0)
        ...
        return JointAttnProcessor2_0Func.apply(
            ...
            self.to_q_weight, self.to_q_bias,
            self.to_k_weight, self.to_k_bias,
            self.to_v_weight, self.to_v_bias,

            self.norm_q_weight, self.norm_q_eps, self.norm_q_elementwise_affine,
            self.norm_k_weight, self.norm_k_eps, self.norm_k_elementwise_affine,

            self.add_q_proj_weight, self.add_q_proj_bias,
            self.add_k_proj_weight, self.add_k_proj_bias,
            self.add_v_proj_weight, self.add_v_proj_bias,

            self.norm_added_q_weight, self.norm_added_q_eps, self.norm_added_q_elementwise_affine,
            self.norm_added_k_weight, self.norm_added_k_eps, self.norm_added_k_elementwise_affine,

            self.to_out_0_weight, self.to_out_0_bias,
            self.to_out_1_dropout_p,

            self.attn_context_pre_only,
            self.attn_added_kv_proj_dim,
            self.to_add_out_weight, self.to_add_out_bias,
...
def JointTransformerBlockOnnx(
    ...
    norm_hidden_states, gate_msa, shift_mlp, scale_mlp, gate_mlp = \
        AdaLayerNormZero(hidden_states, temb, norm1_linear_weight, norm1_linear_bias, norm1_epsilon)

    # ----------------------------------------------------------------------
    # 2. Norm1_context (Fixed to AdaLayerNormZero as context_pre_only is False)
    # ----------------------------------------------------------------------
    norm_encoder_hidden_states, c_gate_msa, c_shift_mlp, c_scale_mlp, c_gate_mlp = \
        AdaLayerNormZero(encoder_hidden_states, temb, norm1_context_linear_weight, norm1_context_linear_bias, norm1_context_epsilon)
    ...
    norm_encoder_hidden_states = RMSNorm(encoder_hidden_states, norm2_context_weight, norm2_context_eps, norm2_context_elementwise_affine)
    norm_encoder_hidden_states = ops.Add(
        ops.Mul(norm_encoder_hidden_states, ops.Add(ops.Constant(value_float=1.0), ops.Unsqueeze(c_scale_mlp, axes=[1]))),
    ...
    encoder_hidden_states = ops.Add(encoder_hidden_states, ops.Mul(ops.Unsqueeze(c_gate_mlp, axes=[1]), context_ff_output))
...
# --- Parameters for attn (JointAttnProcessor2_0) ---
        ...
        attn_inner_dim: int,
        attn_inner_kv_dim: int,
        attn_to_q_weight: torch.Tensor, attn_to_q_bias: torch.Tensor,
        attn_to_k_weight: torch.Tensor, attn_to_k_bias: torch.Tensor,
        attn_to_v_weight: torch.Tensor, attn_to_v_bias: torch.Tensor,
        attn_norm_q_weight: torch.Tensor, attn_norm_q_eps: float, attn_norm_q_elementwise_affine: bool,
        attn_norm_k_weight: torch.Tensor, attn_norm_k_eps: float, attn_norm_k_elementwise_affine: bool,
        attn_add_q_proj_weight: torch.Tensor, attn_add_q_proj_bias: torch.Tensor,
        attn_add_k_proj_weight: torch.Tensor, attn_add_k_proj_bias: torch.Tensor,
        attn_add_v_proj_weight: torch.Tensor, attn_add_v_proj_bias: torch.Tensor,
        attn_norm_added_q_weight: torch.Tensor, attn_norm_added_q_eps: float, attn_norm_added_q_elementwise_affine: bool,
        attn_norm_added_k_weight: torch.Tensor, attn_norm_added_k_eps: float, attn_norm_added_k_elementwise_affine: bool,
        attn_to_out_0_weight: torch.Tensor, attn_to_out_0_bias: torch.Tensor,
        attn_to_out_1_dropout_p: float,
        attn_context_pre_only_flag: bool, # This is the internal flag for JointAttnProcessor2_0
        attn_added_kv_proj_dim: int,
        attn_to_add_out_weight: torch.Tensor, attn_to_add_out_bias: torch.Tensor,
        ...
        ff_context_dim: int, ff_context_dim_out: int, ff_context_mult: int, ff_context_dropout_ratio: float, ff_context_final_dropout: bool,
        ff_context_act_fn_proj_weight: torch.Tensor, ff_context_act_fn_proj_bias: torch.Tensor,
        ff_context_project_out_weight: torch.Tensor, ff_context_project_out_bias: torch.Tensor,
        ...
        norm_hidden_states, gate_msa, shift_mlp, scale_mlp, gate_mlp = \
            AdaLayerNormZeroFunc.apply(hidden_states, temb, norm1_linear_weight, norm1_linear_bias, norm1_epsilon)

        # 2. Norm1_context (AdaLayerNormZero)
        norm_encoder_hidden_states, c_gate_msa, c_shift_mlp, c_scale_mlp, c_gate_mlp = \
            AdaLayerNormZeroFunc.apply(encoder_hidden_states, temb, norm1_context_linear_weight, norm1_context_linear_bias, norm1_context_epsilon)
        ...
        attn_output, context_attn_output = JointAttnProcessor2_0Func.apply(
            ...
            attn_heads, attn_head_dim, attn_scale_qk, attn_scale_val,
            attn_query_dim, attn_inner_dim, attn_inner_kv_dim,
            attn_to_q_weight, attn_to_q_bias,
            attn_to_k_weight, attn_to_k_bias,
            attn_to_v_weight, attn_to_v_bias,
            attn_norm_q_weight, attn_norm_q_eps, attn_norm_q_elementwise_affine,
            attn_norm_k_weight, attn_norm_k_eps, attn_norm_k_elementwise_affine,
            attn_add_q_proj_weight, attn_add_q_proj_bias,
            attn_add_k_proj_weight, attn_add_k_proj_bias,
            attn_add_v_proj_weight, attn_add_v_proj_bias,
            attn_norm_added_q_weight, attn_norm_added_q_eps, attn_norm_added_q_elementwise_affine,
            attn_norm_added_k_weight, attn_norm_added_k_eps, attn_norm_added_k_elementwise_affine,
            attn_to_out_0_weight, attn_to_out_0_bias,
            attn_to_out_1_dropout_p,
            attn_context_pre_only_flag,
            attn_added_kv_proj_dim,
            attn_to_add_out_weight, attn_to_add_out_bias,
        ...
        norm_hidden_states_mlp = RMSNormFunc.apply(current_hidden_states, norm2_weight, norm2_eps, norm2_elementwise_affine)
        ...
        ff_output = FeedForwardFunc.apply(
            ...
            ff_dim, ff_dim_out, ff_mult, ff_dropout_ratio, ff_final_dropout, ff_activation_fn_type,
            ff_act_fn_proj_weight, ff_act_fn_proj_bias,
            ff_project_out_weight, ff_project_out_bias,
        )
        ...
        norm_encoder_hidden_states_mlp = RMSNormFunc.apply(current_encoder_hidden_states, norm2_context_weight, norm2_context_eps, norm2_context_elementwise_affine)
        norm_encoder_hidden_states_mlp = norm_encoder_hidden_states_mlp * (1 + c_scale_mlp.unsqueeze(1)) + c_shift_mlp.unsqueeze(1)

        # Note: `if self._chunk_size is not None` block is skipped.
        context_ff_output = FeedForwardFunc.apply(
            norm_encoder_hidden_states_mlp,
            ff_context_dim, ff_context_dim_out, ff_context_mult, ff_context_dropout_ratio, ff_context_final_dropout, ff_context_activation_fn_type,
            ff_context_act_fn_proj_weight, ff_context_act_fn_proj_bias,
            ff_context_project_out_weight, ff_context_project_out_bias,
        )
    ...
    def symbolic(
        ...
        norm1_context_epsilon: float,
        # Parameters for attn (JointAttnProcessor2_0)
        attn_heads: int, attn_head_dim: int, attn_scale_qk: bool, attn_scale_val: float,
        attn_query_dim: int, attn_inner_dim: int, attn_inner_kv_dim: int,
        attn_to_q_weight: torch.Value, attn_to_q_bias: torch.Value,
        attn_to_k_weight: torch.Value, attn_to_k_bias: torch.Value,
        attn_to_v_weight: torch.Value, attn_to_v_bias: torch.Value,
        attn_norm_q_weight: torch.Value, attn_norm_q_eps: float, attn_norm_q_elementwise_affine: bool,
        attn_norm_k_weight: torch.Value, attn_norm_k_eps: float, attn_norm_k_elementwise_affine: bool,
        attn_add_q_proj_weight: torch.Value, attn_add_q_proj_bias: torch.Value,
        attn_add_k_proj_weight: torch.Value, attn_add_k_proj_bias: torch.Value,
        attn_add_v_proj_weight: torch.Value, attn_add_v_proj_bias: torch.Value,
        attn_norm_added_q_weight: torch.Value, attn_norm_added_q_eps: float, attn_norm_added_q_elementwise_affine: bool,
        attn_norm_added_k_weight: torch.Value, attn_norm_added_k_eps: float, attn_norm_added_k_elementwise_affine: bool,
        attn_to_out_0_weight: torch.Value, attn_to_out_0_bias: torch.Value,
        attn_to_out_1_dropout_p: float,
        attn_context_pre_only_flag: bool,
        attn_added_kv_proj_dim: int,
        attn_to_add_out_weight: torch.Value, attn_to_add_out_bias: torch.Value,
        ...
        ff_act_fn_proj_weight: torch.Value, ff_act_fn_proj_bias: torch.Value,
        ff_project_out_weight: torch.Value, ff_project_out_bias: torch.Value,
        ...
        # Parameters for ff_context (FeedForward)
        ff_context_dim: int, ff_context_dim_out: int, ff_context_mult: int, ff_context_dropout_ratio: float, ff_context_final_dropout: bool,
        ff_context_act_fn_proj_weight: torch.Value, ff_context_act_fn_proj_bias: torch.Value,
        ff_context_project_out_weight: torch.Value, ff_context_project_out_bias: torch.Value,
        ...
        result = g.onnxscript_op(
            ...
            norm1_linear_weight, norm1_linear_bias, norm1_epsilon,
            norm1_context_linear_weight, norm1_context_linear_bias, norm1_context_epsilon,
            ...
            attn_inner_dim_i=attn_inner_dim,
            attn_inner_kv_dim_i=attn_inner_kv_dim,
            attn_to_q_weight=attn_to_q_weight, attn_to_q_bias=attn_to_q_bias,
            attn_to_k_weight=attn_to_k_weight, attn_to_k_bias=attn_to_k_bias,
            attn_to_v_weight=attn_to_v_weight, attn_to_v_bias=attn_to_v_bias,
            attn_norm_q_weight=attn_norm_q_weight, attn_norm_q_eps_f=attn_norm_q_eps, attn_norm_q_elementwise_affine_i=attn_norm_q_elementwise_affine,
            attn_norm_k_weight=attn_norm_k_weight, attn_norm_k_eps_f=attn_norm_k_eps, attn_norm_k_elementwise_affine_i=attn_norm_k_elementwise_affine,
            attn_add_q_proj_weight=attn_add_q_proj_weight, attn_add_q_proj_bias=attn_add_q_proj_bias,
            attn_add_k_proj_weight=attn_add_k_proj_weight, attn_add_k_proj_bias=attn_add_k_proj_bias,
            attn_add_v_proj_weight=attn_add_v_proj_weight, attn_add_v_proj_bias=attn_add_v_proj_bias,
            attn_norm_added_q_weight=attn_norm_added_q_weight, attn_norm_added_q_eps_f=attn_norm_added_q_eps, attn_norm_added_q_elementwise_affine_i=attn_norm_added_q_elementwise_affine,
            attn_norm_added_k_weight=attn_norm_added_k_weight, attn_norm_added_k_eps_f=attn_norm_added_k_eps, attn_norm_added_k_elementwise_affine_i=attn_norm_added_k_elementwise_affine,
            attn_to_out_0_weight=attn_to_out_0_weight, attn_to_out_0_bias=attn_to_out_0_bias,
            attn_to_out_1_dropout_p_f=attn_to_out_1_dropout_p,
            attn_context_pre_only_flag_i=attn_context_pre_only_flag,
            attn_added_kv_proj_dim_i=attn_added_kv_proj_dim,
            attn_to_add_out_weight=attn_to_add_out_weight, attn_to_add_out_bias=attn_to_add_out_bias,
            
            norm2_weight=norm2_weight, norm2_eps_f=norm2_eps, norm2_elementwise_affine_i=norm2_elementwise_affine,
            ff_dim_i=ff_dim, ff_dim_out_i=ff_dim_out, ff_mult_i=ff_mult, ff_dropout_ratio_f=ff_dropout_ratio, ff_final_dropout_i=ff_final_dropout,
            ff_act_fn_proj_weight=ff_act_fn_proj_weight, ff_act_fn_proj_bias=ff_act_fn_proj_bias,
            ff_project_out_weight=ff_project_out_weight, ff_project_out_bias=ff_project_out_bias,
            ff_activation_fn_type_i=ff_activation_fn_type,

            norm2_context_weight=norm2_context_weight, norm2_context_eps_f=norm2_context_eps, norm2_context_elementwise_affine_i=norm2_context_elementwise_affine,

            ff_context_dim_i=ff_context_dim, ff_context_dim_out_i=ff_context_dim_out, ff_context_mult_i=ff_context_mult, ff_context_dropout_ratio_f=ff_context_dropout_ratio, ff_context_final_dropout_i=ff_context_final_dropout,
            ff_context_act_fn_proj_weight=ff_context_act_fn_proj_weight, ff_context_act_fn_proj_bias=ff_context_act_fn_proj_bias,
            ff_context_project_out_weight=ff_context_project_out_weight, ff_context_project_out_bias=ff_context_project_out_bias,
